...
NEGATIVE_EXCLUDE = {
    ...
    "🍉果切相关": [
        ...
        "正太",        # 「正太」指人物（“正太苹果”是人名/昵称），非水果
        "苹果价格",    # 苹果公司产品/股价语境（“苹果价格把我的购物欲治好了”），非水果
    ],
}
...
NEGATIVE_REGEX = {
    "🍉果切相关": [
        ...
        re.compile(r"芒果(?:夜|台|tv|视频|网络|超媒|娱乐|影像|剧)", re.IGNORECASE),  # 芒果TV/芒果台/青春芒果夜 等品牌
        # 芒果TV 影视剧/综艺语境（"芒果一口气推出89部大剧"等），非水果
        re.compile(r"芒果.{0,10}(?:剧|综艺|开播|播出|首播|定档|上线)", re.IGNORECASE),
        # 苹果公司科技语境：苹果与手机/数码/发布/新品等词同现（非连续），规避「苹果首款折叠屏手机」类误伤
        re.compile(
            r"苹果.{0,8}(?:手机|折叠|屏幕|屏|新品|发布|电脑|笔记本|平板|手表|耳机|芯片|系统|"
            r"汽车|首款|新款|新机|卡托|亮相|上新|系列|官方|官网|价格|股价|市值|税|表|闹钟|铃|Pro|Max|Ultra|iPhone|"
            r"Siri|人工智能|AI|破发|直降|财报|销量|出货|概念股|股票)",
            re.IGNORECASE,
        ),
        # 反向顺序：手机/折叠屏/新品等词在前、苹果在后（如「折叠屏苹果」）
        re.compile(
            r"(?:手机|折叠屏|折叠|电脑|平板|手表|耳机|芯片|发布|新品|首款|新款|iPhone).{0,8}苹果",
            re.IGNORECASE,
        ),
        ...
        # 水果当凶器/涉法纠纷语境（"被顾客用西瓜砸头摊主获赔近7万"），非果切经营
        re.compile(r"(?:西瓜|榴莲|芒果|苹果|椰子|哈密瓜|葡萄|梨).{0,8}(?:砸|砸头|砸伤|获赔|判赔|赔偿|斗殴|打架|纠纷|诉讼)", re.IGNORECASE),
    ],
}
